...
class QuizBrain {
  int _number = 0;
  List<QuizInformation> _questionbank = [
    QuizInformation('달팽이도 이빨이 있다.', true),
  ...
  void currentNumber(context, int rightAnswer) {
    if (_number < _questionbank.length - 1) {
      _number++;
    } else if (_number == _questionbank.length - 1) {
      Alert(
        context: context,
        title: '퀴즈 종료',
        desc: "맞춘 정답의 갯수는 $rightAnswer개 입니다",
        buttons: [
          DialogButton(
              child: Text('다시시작'),
              onPressed: () {
